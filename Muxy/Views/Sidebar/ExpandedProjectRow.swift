...
struct ExpandedProjectRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            projectHeader
            if worktreesExpanded, vcsKind != nil {
                worktreeList
            }
        }
        .task(id: project.path) {
            vcsKind = await VCSKind.detect(at: project.path)
            if autoExpandWorktrees, isActive, vcsKind != nil {
                worktreesExpanded = true
            }
        }
        .onChange(of: isActive) { _, active in
            guard autoExpandWorktrees, active, vcsKind != nil else { return }
    ...
    private var projectHeader: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                if vcsKind != nil, let worktree = activeWorktree {
